...
if ! grep -q 'texlive/bin' ~/.bashrc 2>/dev/null; then
  echo 'export PATH="$HOME/texlive/bin/x86_64-linux:$PATH"' >> ~/.bashrc
fi

mkdir -p ~/.config/fish
if ! grep -q 'texlive/bin' ~/.config/fish/config.fish 2>/dev/null; then
  echo 'fish_add_path $HOME/texlive/bin/x86_64-linux' >> ~/.config/fish/config.fish
fi
...
tlmgr install \
      ...
      mptopdf mptopdf.x86_64-linuxmusl multirow natbib newpx newtx norasi-c90 \
